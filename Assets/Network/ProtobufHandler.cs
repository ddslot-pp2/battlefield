...
public class ProtobufHandler //: MonoBehaviour
{
    ...
    public void SetDeserializer()
    {
        ...
        Deserializer_[OpcodeToInt16(opcode.SC_PURCHASE_CHARACTER)] = (byte[] Buf) =>
        {
        ...
        Deserializer_[OpcodeToInt16(opcode.SC_MY_INFO)] = (byte[] Buf) =>
        {
            return LOBBY.SC_MY_INFO.Parser.ParseFrom(Buf);
        };

        Deserializer_[OpcodeToInt16(opcode.SC_MY_CHARACTER_INFO)] = (byte[] Buf) =>
        {
            return LOBBY.SC_MY_CHARACTER_INFO.Parser.ParseFrom(Buf);
        };

        Deserializer_[OpcodeToInt16(opcode.SC_CHARACTER_UPGRADE)] = (byte[] Buf) =>
        {
            return LOBBY.SC_CHARACTER_UPGRADE.Parser.ParseFrom(Buf);
        };

        Deserializer_[OpcodeToInt16(opcode.SC_CHARACTER_SELECT)] = (byte[] Buf) =>
        {
            return LOBBY.SC_CHARACTER_SELECT.Parser.ParseFrom(Buf);
        };
