...
public class FireTypeBallBullet : BallBullet {
    void Start()
    {
        radius = 10;
    ...
    {
        //충돌한 게임오브젝트의 태그값 비교
        if (other.transform.tag == "Tank" || other.transform.tag == "Soldier" || other.transform.tag == "DestroyObject")
        {
            if (other.transform.tag == "Tank")
            {
                BulletDamageManager.Instance.GetDamage(damage, other.gameObject, attacker);
                if (Random.Range(1, 100) >= 50)
                    BulletDamageManager.Instance.GetFireEffect(other.gameObject, 10, attacker);
            }
