...
public class Alisa_TopFire : MonoBehaviour {
    //총알 발사좌표
    ...
    void Start()
    {
        state = gameObject.GetComponentInParent<Alisa_State>();
    ...
    void CreateBullet()
    {
        ...
        bulletLocalSize.GetComponent<BallBullet>().GetDamageType(state.damage, 3, transform.parent.gameObject, state.range, state.bulletSpeed);
    }
