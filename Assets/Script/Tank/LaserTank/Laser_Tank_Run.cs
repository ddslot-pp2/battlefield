...
public class Laser_Tank_Run : MonoBehaviour {
    ...
    void Start()
    {
        ...
        state = gameObject.GetComponent<Laser_Tank_State>();
    }
    ...
    void Update()
    {
        if (controller.dir.x != 0 && controller.dir.y != 0)
        {
            ...
            playertank.rotation = Quaternion.LookRotation(lookDirection);
            if (controller.dir != new Vector3(0, 0, 0))
                playertank.Translate(Vector3.forward * state.moveSpeed * Time.deltaTime);
        }
    }
}
